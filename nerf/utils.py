...
def save_videos_grid(videos: torch.Tensor, path: str, n_rows=8, fps=8, **kwargs):
    ## videos b f c h w
...
def one_video_save(save_path, video, **kwargs):
    # ipdb.set_trace()
    video = video.detach().cpu()
    if video.ndim == 4:
        imageio.mimwrite(save_path, video.numpy(), fps=25, quality=8, macro_block_size=1)
    elif video.ndim == 5:
        B,F = video.size()[:2]
        # Initialize an empty list to store the extracted tensors
        extracted_tensors = []
        # Iterate over batch indices

        for batch_index in range(B):
            # Calculate the index based on the rule (batch index modulo F)
            index_to_extract = batch_index % F
            
            # Extract the tensor for the current batch index and calculated index
            extracted_tensor = video[batch_index, index_to_extract] # H,W,3
        ...
        result_tensor = torch.stack(extracted_tensors)
        imageio.mimwrite(save_path, result_tensor.numpy(), fps=25, quality=8, macro_block_size=1)
