...
from transformers import CLIPFeatureExtractor, CLIPTextModel, CLIPTokenizer, CLIPVisionModelWithProjection, CLIPImageProcessor
import einops
...
import numpy as np
import math
from .cc_projector import ProjectionModel
...
class Zero1to3StableDiffusionPipeline(DiffusionPipeline, FromSingleFileMixin):
    ...
    def _encode_image(self, image, device, num_images_per_prompt, do_classifier_free_guidance):
        ...
        image_embeddings = self.image_encoder(image.to(self.image_encoder.device)).image_embeds.to(dtype=dtype)
    ...
    def decode_latents(self, latents):
        ...
        image = image.cpu().permute(0, 2, 3, 1).float().numpy() # B,H,W,3
        return image
    ...
    def __call__(
        ...
        if output_type == "latent":
            ...
            image = self.decode_latents(latents)
            image = torch.from_numpy(image) # B,H,W,3
        else:
            # 8. Post-processing
            image = self.decode_latents(latents)
