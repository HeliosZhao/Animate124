...
import torch.nn.functional as F
from diffusers import AutoencoderKL, UNet2DConditionModel, PNDMScheduler, DDIMScheduler, StableDiffusionPipeline, StableDiffusionImg2ImgPipeline, DiffusionPipeline
    ...
    def train_step(self, text_embeddings, pred_rgb, guidance_scale=100, as_latent=False, grad_clip=None, grad_scale=1.0,
        ...
        if as_latent:
            latents = F.interpolate(
                pred_rgb, (sd_size//8, sd_size//8), mode='bilinear', align_corners=False) * 2 - 1
        else:
            ...
            if video_generation:
                latents = rearrange(latents, "(b f) c h w -> b c f h w", f=num_frame) 


        t = self.noise_sample(step=step, shape=latents.shape[0]).to(self.device)
        ...
        if self.new_sds:
            ...
            if self.opt.get('mean_sds', False):
                ...
                ## add frame level mean
                factor = latents.shape[0]
                if video_generation and self.opt.mean_frame:
                    factor = factor * num_frame
                loss = 0.5 * F.mse_loss(latents.float(), targets, reduction='sum') / factor # B,4,H,W -> sum over 4,H,W
        ...
        if save_guidance_path:
            with torch.no_grad():
                ...
                images = []
                if video_generation:
                    pred_rgb_512 = rearrange(pred_rgb_512,  "(b f) c h w -> b c f h w", f=num_frame)
                ...
                for t in timesteps:
                    ...
                    pred_original_sample = self.decode_latents((latents_noisy - (1 - self.alphas[t]) ** (0.5) * noise_pred) / self.alphas[t] ** (0.5), video_generation)

                    # visualize predicted denoised image
                    # claforte: discuss this with Vikram!!
                    result_hopefully_less_noisy_image = self.decode_latents(latents - w*(noise_pred - noise), video_generation)

                    # visualize noisier image
                    result_noisier_image = self.decode_latents(latents_noisy, video_generation) 
                    ...
                    noise_pred = noise_pred_text + guidance_scale * \
                            (noise_pred_text - noise_pred_uncond)
                    noise_diffusion_out = self.decode_latents((noise - (1 - self.alphas[t]) ** (0.5) * noise_pred) / self.alphas[t] ** (0.5), video_generation)
                ...
                if video_generation:
                    ...
                    save_tensor2image(rearrange(viz_images, 'b c f h w -> b f c h w'), save_guidance_path, n_rows=5)
                else:
