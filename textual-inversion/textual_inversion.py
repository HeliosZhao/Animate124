...
def save_images_as_grid(images, output_filename, nrow=4, padding=2):
    """
    Save a list of images as a grid.

    :param images: List of PIL Image objects.
    :param nrow: number of images displayed in one row.
    :param output_filename: Filename to save the grid image.
    :param padding: Optional padding between images.
    """

    # Calculate the dimensions of the grid
    img_width, img_height = images[0].size
    grid_shape = (len(images)//nrow+int(len(images)%nrow), nrow )
    grid_width = grid_shape[1] * img_width + (grid_shape[1] - 1) * padding
    grid_height = grid_shape[0] * img_height + (grid_shape[0] - 1) * padding 

    # Create a blank white image for the grid
    grid_image = Image.new('RGB', (grid_width, grid_height), 'white')

    # Paste each image into its position on the grid
...
def log_validation(text_encoder, tokenizer, unet, vae, args, accelerator, weight_dtype, epoch, step):
    logger.info(
    ...
    valid_dir = os.path.join(args.output_dir, "validation")
    ...
    save_images_as_grid(images, os.path.join(valid_dir, f'step-{str(step).zfill(4)}.jpg'))
