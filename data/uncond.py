...
class RandomCameraIterableDataset(IterableDataset, Updateable):
    ...
    def collate(self, batch) -> Dict[str, Any]:
        ...
        if self.cfg.sample_rand_frames == "t0":
        ...
        elif self.cfg.sample_rand_frames == "fps":
            zero_rate = self.cfg.zero_rate
            full_precision = self.cfg.precision
            ...
            if start_from_zero:
                ...
                start_time = random.randint(0, precision-frame_time) 

            time = ((torch.arange(num_frames) * time_interval) + start_time) / precision * full_precision

            if random.random() > 0.5:
                time = full_precision - time
                time = time.tolist()
                time = torch.FloatTensor(time[::-1])
                start_from_zero = False

            frame_times = time.float() / full_precision
            assert frame_times.max() <= 1 and frame_times.min()>=0 and frame_times.size(0) == num_frames
...
@register("dynamic-random-camera-datamodule")
